...
class BackupsPageUI(wx.Panel, BasePageUI):
    ...
    def __init__(self, parent: wx.Window):
        ...
        self._backup_now_button = wx.Button(self, label="Backup Now")
        self._backup_now_button.Bind(wx.EVT_BUTTON, self._backup_now)
        root.Add(self._backup_now_button, 0, wx.ALL | wx.ALIGN_RIGHT, 10)
    ...
    def _backup_now(self, _evt):
        backup_folder = self._backup_folder_text.GetValue().strip()
        worlds_folder = self._worlds_folder_text.GetValue().strip()

        if not backup_folder:
            wx.MessageBox("Backup folder is required.", "Backup Error", wx.OK | wx.ICON_ERROR)
            return
        if not worlds_folder:
            wx.MessageBox("Minecraft worlds folder is required.", "Backup Error", wx.OK | wx.ICON_ERROR)
            return
        if not os.path.isdir(worlds_folder):
            wx.MessageBox(
                "Minecraft worlds folder does not exist.",
                "Backup Error",
                wx.OK | wx.ICON_ERROR,
            )
            return
        ...
        try:
            ...
            if os.path.exists(zip_path):
                try:
                    ...
                    pass
            wx.MessageBox(f"Backup failed:\n{e}", "Backup Error", wx.OK | wx.ICON_ERROR)
        else:
            wx.MessageBox(f"Backup created:\n{zip_path}", "Backups", wx.OK | wx.ICON_INFORMATION)
        finally:
            progress.Destroy()
